...
ThalassaAgent.prototype.handleThalassaOnline = function (service) {
  var self = this;
  var activity = { type: 'activity',  time: Date.now(), verb: 'online', object: service.id };
...
ThalassaAgent.prototype._connectToAqueduct = function(service) {
  ...
  var mx = new MuxDemux(function (s) {
    if (s.meta.type === 'config') {
      ...
      .on('data', function (line) {
        try {
          var activityObj = JSON.parse(line);
          self.emit('activity', activityObj);
        } catch(err) {
          self.log('error', 'error parsing activity from stream ' + line, String(err));
        }
      })
      .on('error', function () { s.destroy(); });
    }
  ...
  for (var id in this._clientMuxes) {
...
ThalassaAgent.prototype.handleThalassaOffline = function (id) {
  var self = this;
  var activity = { type: 'activity', time: Date.now(), verb: 'offline', object: id };
  self.emit('activity', activity);
